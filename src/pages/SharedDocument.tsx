...
const SharedDocument = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get(`/api/shared-documents/${shareToken}`);
      const data = response.data;

      if (!data?.share || !data?.document) {
        throw new Error("Share link not found or expired");
      }

      if (data.share.expires_at && new Date(data.share.expires_at) < new Date()) {
  ...
  const trackAccessMutation = useMutation({
    mutationFn: async (shareId: string) => {
      await api.put(`/api/document-shares/${shareId}`, {
