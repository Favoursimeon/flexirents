...
export default function VerificationManagement() {
  ...
  const fetchSubmissions = async () => {
    try {
      const response = await api.get("/api/admin/user-verifications", {
        params: { orderBy: "created_at", orderDirection: "desc" },
      });
      const data = response.data || [];
      const userIds = data.map((d: any) => d.user_id) || [];

      const profilesResponse = await api.get("/api/admin/users/profiles", {
  ...
  const handleApprove = async (submissionId: string) => {
    ...
    try {
      ...
      await api.put(`/api/admin/user-verifications/${submissionId}/approve`, {
        income_source: submission.employment_status || "informal",
      });
  ...
  const handleReject = async (submissionId: string) => {
    ...
    setProcessing(true);
    try {
      await api.put(`/api/admin/user-verifications/${submissionId}/reject`, {
