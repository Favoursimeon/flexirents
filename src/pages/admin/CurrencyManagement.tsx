...
export default function CurrencyManagement() {
  ...
  const fetchCurrencies = async () => {
    try {
      const response = await api.get("/api/admin/currency-rates", {
        params: { orderBy: "currency_code" },
      });
  ...
  const handleSave = async (currencyId: string) => {
    ...
    try {
      await api.put(`/api/admin/currency-rates/${currencyId}`, { rate_to_usd: newRate });
