...
const Index = () => {
  ...
  useEffect(() => {
    const fetchRecentProperties = async () => {
      try {
        const { data } = await api.get('/api/properties?status=available&limit=5');
  ...
  useEffect(() => {
    const fetchRentals = async () => {
      try {
        const { data } = await api.get('/api/properties?status=available&listing_type=rent&limit=3');
        if (data && data.length > 0) {
  ...
  useEffect(() => {
    const fetchSales = async () => {
      try {
        const { data } = await api.get('/api/properties?status=available&listing_type=sale&limit=3');
        if (data && data.length > 0) {
      ...
      try {
        ...
        if (data && data.length > 0) {
          const categoryCounts = data.reduce((acc: any, vendor: any) => {
            ...
            if (!acc[category]) acc[category] = { count: 0, category };
            acc[category].count += 1;
            return acc;
          }, {});
          const categories = Object.entries(categoryCounts)
            .map(([category, d]: [string, any]) => ({
              id: category,
              title: category,
          ...
          setVendorCategories(defaultVendorCategories);
        }
      } catch (error) {
        setVendorCategories(defaultVendorCategories);
      } finally {
    ...
    <div className="min-h-screen">
      ...
      <section className="relative pt-16 min-h-[600px] flex items-center">
        <div className="absolute inset-0 z-0">
          <img src={heroImage} alt="Modern luxury property" className="w-full h-full object-cover" />
            ...
            {loadingProperties ? (
              ...
              <div className="bg-background/95 backdrop-blur-sm rounded-xl p-6 max-w-2xl">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="icon" onClick={prevProperty} className="h-9 w-9 bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg">
                      <ChevronLeft className="h-5 w-5" />
                    </Button>
                    <Button size="icon" onClick={nextProperty} className="h-9 w-9 bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg">
                      <ChevronRight className="h-5 w-5" />
                    </Button>
                  </div>
                </div>
                <div className="relative overflow-hidden">
                  <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentPropertyIndex * 100}%)` }}>
                      ...
                      return (
                        <div key={property.id || index} className="min-w-full">
                          <div className="flex items-center gap-4">
                            <img src={propertyImage} alt={property.title} className="w-24 h-24 object-cover rounded-lg" />
                            <div className="flex-1">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <Button size="sm" variant="secondary" onClick={() => navigate(propertyType === "rent" ? "/rentals" : "/sales")}>
                                  View Details
                                </Button>
                              </div>
                            </div>
            ...
            <>
              ...
              <div className="text-center">
                <Button variant="default" size="lg" asChild>
                  <Link to="/rentals">Explore All Rentals <ArrowRight className="ml-2 h-4 w-4" /></Link>
                </Button>
              </div>
            </>
          )}
        ...
        <div className="container mx-auto px-4">
          ...
          {loadingSales ? (
            ...
            <>
              ...
              <div className="text-center">
                <Button variant="default" size="lg" asChild>
                  <Link to="/sales">View All Properties <ArrowRight className="ml-2 h-4 w-4" /></Link>
                </Button>
              </div>
            </>
          )}
      ...
      <section className="py-10 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              Connect with verified vendors for electronics, construction materials, and more.
            </p>
          </div>
          {loadingVendors ? (
            <div className="flex items-center justify-center py-12">
            ...
            <>
              ...
              <div className="text-center">
                <Button variant="default" size="lg" asChild>
                  <Link to="/marketplace">Browse All Vendors <ArrowRight className="ml-2 h-4 w-4" /></Link>
                </Button>
              </div>
            </>
          )}
      ...
      <section className="py-10 bg-gradient-to-b from-muted/30 to-muted/60">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {[
              { initials: "SA", name: "Sarah Anderson", role: "Apartment Renter", quote: "Finding my dream apartment was so easy with FlexiRents. The flexible payment plan made it affordable, and the team was incredibly supportive throughout." },
              { initials: "MO", name: "Michael Okonkwo", role: "Property Investor", quote: "Purchased my first commercial property through FlexiRents. The process was transparent, and their guidance helped me make the right investment decision." },
              { initials: "EA", name: "Emeka Afolabi", role: "Happy Tenant", quote: "FlexiRents made finding my dream home incredibly easy. The platform is intuitive and the support team was always responsive." },
            ].map((story) => (
              <div key={story.initials} className="bg-card p-8 rounded-xl shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-card-hover)] transition-all border border-border/50">
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="grid md:grid-cols-2 gap-6 text-left">
              {[
                { title: "Verified Properties", desc: "All properties undergo thorough verification before listing." },
                { title: "Transparent Pricing", desc: "Clear pricing with no hidden fees. What you see is what you pay." },
                { title: "Vetted Vendors", desc: "All marketplace vendors are verified and rated by the community." },
                { title: "24/7 Support", desc: "Our team is always available to assist with your needs." },
              ].map((item) => (
