...
const VendorProfile = () => {
  ...
  const fetchVendorData = async () => {
    try {
      ...
      const ratingResponse = await api.get(`/api/vendors/${id}/rating`);
      setAverageRating(ratingResponse.data?.averageRating || 0);

      const countResponse = await api.get(`/api/vendors/${id}/review-count`);
  ...
  const fetchProducts = async () => {
    try {
      const response = await api.get(`/api/vendors/${id}/products`, {
        params: { orderBy: 'is_featured_desc,created_at_desc' },
      });
  ...
  const fetchReviews = async () => {
    try {
      const response = await api.get(`/api/vendors/${id}/reviews`, {
