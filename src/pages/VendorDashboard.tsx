...
const VendorDashboard = () => {
  ...
  const fetchVendorData = async () => {
    try {
      ...
      const ratingResponse = await api.get(`/api/vendors/${data.id}/rating`);
      setAverageRating(ratingResponse.data?.averageRating || 0);

      const countResponse = await api.get(`/api/vendors/${data.id}/review-count`);
      setReviewCount(countResponse.data?.reviewCount || 0);
    } catch (error: any) {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await api.get(`/api/vendors/${vendor.id}/products`, {
  ...
  const fetchReviews = async () => {
    ...
    try {
      const response = await api.get(`/api/vendors/${vendor.id}/reviews`, {
        params: { orderBy: 'created_at_desc', includeReviewer: true },
      });
  ...
  const onSubmit = async (data: ProfileFormData) => {
    ...
    setUpdating(true);
    try {
      await api.put(`/api/vendors/${vendor.id}`, {
