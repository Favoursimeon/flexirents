...
export default function RentalBillingHistory() {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      if (pending.length > 0) setNextDueDate(pending[0].due_date);
    } catch (error: any) {
  ...
  const handleDownloadReceipt = async (payment: RentalPayment) => {
    ...
    try {
      ...
      doc.text("FlexiRents - Rental Payment System", 105, 30, { align: 'center' });
      ...
      doc.save(`receipt-installment-${payment.installment_number}.pdf`);
  ...
  const monthsPaidVerified = payments
    ...
    .reduce((total, p) => total + (p.is_first_payment ? firstPaymentMonths : 1), 0);
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-green-600">{formatPrice(payments.filter(p => p.status === "paid").reduce((sum, p) => sum + p.amount, 0))}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-red-600">{formatPrice(payments.filter(p => p.status === "overdue").reduce((sum, p) => sum + p.amount, 0))}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold">{monthsPaidVerified}<span className="text-sm text-muted-foreground">/{leaseDurationMonths}</span></p>
            </div>
          </CardContent>
        </Card>
      </div>
      {nextDueDate && (
        <Alert className="bg-yellow-500/10 border-yellow-500/50">
          ...
          <AlertDescription className="text-sm">
            You have a payment due on <strong>{format(new Date(nextDueDate), "MMMM dd, yyyy")}</strong>. Please ensure timely payment.
          </AlertDescription>
        </Alert>
      )}
      <Card>
        <CardHeader>
          ...
          <CardDescription>View all your rental payment transactions and download receipts</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {displayedPayments.map((payment) => {
                    ...
                    return (
                      <TableRow key={payment.id} className={isNextPending ? 'bg-blue-50/50 dark:bg-blue-950/20 border-l-4 border-l-blue-500' : ''}>
                        <TableCell>
                          <span className="font-medium">
                            {payment.is_first_payment ? "First (6-12mo)" : isNextPending ? "Next Due" : `#${payment.installment_number}`}
                          </span>
                        </TableCell>
                        <TableCell className="font-medium">{format(new Date(payment.due_date), "MMM dd, yyyy")}</TableCell>
                        ...
                        <TableCell>
                          {payment.payment_date ? format(new Date(payment.payment_date), "MMM dd, yyyy") : <span className="text-muted-foreground">Pending</span>}
                        </TableCell>
                        <TableCell>
                          {payment.status === 'paid' ? (
                            <Button variant="ghost" size="sm" onClick={() => handleDownloadReceipt(payment)} className="h-8">
                              <Download className="h-4 w-4 mr-1" />Receipt
                            </Button>
                          ) : payment.status === 'pending' ? (
                            <Button size="sm" disabled={!isNextPending} onClick={() => handlePayment(payment.id)} className={isNextPending ? "bg-blue-600 hover:bg-blue-700 h-8" : "h-8"}>
