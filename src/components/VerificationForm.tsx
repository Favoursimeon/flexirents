...
export default function VerificationForm() {
  ...
  const fetchVerification = async () => {
    ...
    try {
      const { data } = await api.get<VerificationData>('/api/user-verification', {
  ...
  const uploadFile = async (file: File, path: string) => {
    ...
    const { data } = await api.post<{ publicUrl: string }>('/api/verification/upload', formData, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/user-verification', {
        user_id: user.id,
        ...verification,
        status: "pending",
      });
