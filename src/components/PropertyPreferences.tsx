...
export default function PropertyPreferences() {
  ...
  const fetchPreferences = async () => {
    ...
    try {
      const response = await api.get(`/api/user-preferences/${user.id}`);
  ...
  const fetchMatchingCount = async () => {
    ...
    try {
      const response = await api.post("/api/properties/count", {
        status: "available",
        filters: preferences,
      });
      setMatchingCount(response.data?.count || 0);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await api.put(`/api/user-preferences/${user.id}`, preferences);
